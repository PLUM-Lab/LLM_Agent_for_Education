...
def write_rag_config(port):
    """Write rag-config.js so HTML pages know which port the RAG server is on."""
    config_path = Path(__file__).resolve().parent / "rag-config.js"
    config_path.write_text(
        f"// RAG server port configuration\n"
        f"// Auto-generated by start.py -- do not edit manually.\n"
        f"window.RAG_PORT = {port};\n",
        encoding="utf-8"
    )
    print(f"[Config] rag-config.js written with RAG_PORT={port}")
...
def restart_rag_server(port=5001):
...
Examples:
  ...
  python start.py --ui --port 8000 --evaluator --evaluator-port 8002
    ...
    parser.add_argument('--rag-port', type=int, default=5001,
                       help='RAG server port (default: 5001)')
    ...
    # Write rag-config.js so HTML pages know the RAG port
    write_rag_config(args.rag_port)
    ...
    if args.rag and not is_linux and not args.no_wsl_rag:
        ...
        if rag_wsl_process is not None:
            print(f"  RAG server starting in WSL (port {args.rag_port})...")
            time.sleep(3)  # WSL init
            # Set up port forwarding so localhost:<rag_port> -> WSL:<rag_port> (may trigger UAC once)
        ...
        # Clear port proxy so the chosen rag_port hits local RAG (not dead WSL); needs admin
        try:
            r = subprocess.run(
                ["netsh", "interface", "portproxy", "delete", "v4tov4",
                 f"listenport={args.rag_port}", "listenaddress=127.0.0.1"],
                capture_output=True, timeout=5,
            )
            if r.returncode != 0:
                print(f"  [Tip] If Tutor fails, open http://<your-LAN-IP>:{args.port}/medical-quiz.html\n")
        except Exception:
            print(f"  [Tip] If Tutor fails, open UI via LAN IP, e.g. http://192.168.1.16:{args.port}/medical-quiz.html\n")
    ...
    if args.rag:
        ...
        print(f"  RAG (http://127.0.0.1:{args.rag_port}) -> {'OK' if ok else 'NO RESPONSE'}")
    if args.evaluator:
